...
$(function () {
    ...
    $('.oUl li').each(function (index) {
        ...
        $(this).on('mouseover', function () {
            $('.oUl li span').eq(i).stop().animate({top: 0});
            $('.oUl li a').eq(i).toggleClass('current').siblings().removeClass('current');
            audio.pause();
            audio.play();

        });
        $(this).on('mouseout', function () {
            $('.oUl li span').eq(i).stop().animate({top: 35});
            $('.oUl li a').removeClass('current');
        });
    });
    /**
     * 点击切换小红心
